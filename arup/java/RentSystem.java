...
class RentalAgency {
    ...
    public boolean addVehicle(Vehicle vehicle) {
        if (vehicleFleet.containsKey(vehicle.getName().toLowerCase())) {
            return false; // Indicate failure: vehicle already exists
        }
        vehicleFleet.put(vehicle.getName().toLowerCase(), vehicle);
        return true; // Indicate success
    }

    public Collection<Vehicle> getFleet() {
        return vehicleFleet.values();
    }
...
class RentalService {
    ...
    private void printFleetReport() {
        System.out.println("\n--- Vehicle Fleet Status ---");
        Collection<Vehicle> fleet = agency.getFleet();
        if (fleet.isEmpty()) {
...
public class RentSystem {
    public static void main(String[] args) {
        ...
        // The addVehicle method now returns a boolean, though we don't need to check it for this initial setup.
